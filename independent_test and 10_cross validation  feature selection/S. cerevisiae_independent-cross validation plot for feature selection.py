#!/usr/bin/env python
# -*- coding:utf-8-*-
# auther:kewei Liu time:2019/3/18 14:27  QQ:422209303  e-mail:Liukeweiaway@hotmail.com
# --------------------------------------------------------------------------
...
# ##文件输入
# mrmd90个排序
# selection_feature_index = [38, 82, 44, 53, 64, 75, 67, 76, 65, 35, 74, 27, 55, 52, 72, 85, 81, 50, 66, 45,
#                            80, 63, 84, 42, 73, 62, 83, 87, 70, 46, 90, 59, 88, 79, 56, 86, 9, 69, 49, 37,
#                            1, 77, 40, 33, 10, 71, 32, 61, 68, 89, 13, 25, 14, 48, 7, 3, 19, 36, 22, 41]
# mrmd2.0 80feature排序
# selection_feature_index = [38, 82, 44, 53, 64, 75, 67, 76, 65, 35, 74, 27, 55, 52, 72, 85, 81, 50, 66, 45,
#                            80, 63, 84, 42, 73, 62, 83, 87, 70, 46, 90, 59, 88, 79, 56, 86, 9, 69, 49, 37,
#                            1, 77, 40, 33, 10, 71, 32, 61, 68, 89, 13, 25, 14, 48, 7, 3, 19, 36, 22, 41,
#                            20, 57, 26, 51, 4, 43, 5, 31, 29, 54, 47, 60, 12, 21, 24, 23, 78, 34, 18, 15]
# mrmr90个排序  # 41 73
# selection_feature_index = [74, 18, 66, 82, 63, 73, 78, 70, 85, 79, 14, 88, 72, 68, 75, 64, 69, 81, 89, 45, 83, 62, 77,
#                            67, 71, 44, 86, 90, 76, 80, 87, 84, 65, 48, 1, 43, 34, 40, 49, 32, 15, 30]
selection_feature_index = [74, 18, 66, 82, 63, 73, 78, 70, 85, 79, 14, 88, 72, 68, 75, 64, 69, 81, 89, 45, 83, 62, 77,
                           67, 71, 44, 86, 90, 76, 80, 87, 84, 65, 48, 1, 43, 34, 40, 49, 32, 15, 30, 52, 37, 42, 13,
                           46, 50, 61, 25, 59, 7, 55, 60, 5, 21, 54, 57, 38, 47, 56, 39, 36, 53, 28, 17, 41, 33, 51, 2,
                           58, 35, 12, 10, 24, 11, 31, 27, 9, 3, 22, 8, 19, 4, 29, 26, 20, 23, 6, 16]
length = len(selection_feature_index)  # 因为是从1开始就是两个特征开始，要减去1
for selection in range(1, len(selection_feature_index), 1):
    pddtrain = pd.read_csv("627_sc_samples_90_psxp_new_out", sep=' ', header=None)  # train data
    X_train = pddtrain.values[:, selection_feature_index[0:selection]]
    y_train = list(pddtrain.iloc[:, 0])
    pddtest = pd.read_csv("200_sc_samples_90_psxp_new_out", sep=' ', header=None)  # predict data
...
# -----------------------------------------------------测试集最大值点的坐标-----------------------------
max_list_10 = [max(RF_list_10), max(knn_list_10), max(naiveB_list_10), max(svm_list_10), max(LR_list_10), max(eclf_list_10)]
...
plt.annotate((feature_number_10-1, round(max(max_list_10), 3)), xy=(feature_number_10, max(max_list_10)), xycoords='data', xytext=(-30, +30),
             textcoords='offset points', fontsize=16,
             arrowprops=dict(arrowstyle='->', connectionstyle="arc3,rad=.2"))
plt.xlim((0, length+1))
plt.ylim((0.6, 1))
